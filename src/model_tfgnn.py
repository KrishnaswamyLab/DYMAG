...
"""
From https://github.com/vasile-paskardlgm/TFGNN/blob/master/networks.py
"""
...
class Trigo_prop(MessagePassing):
    ...
    def __init__(self, K, omega, D, **kwargs):
        ...
        # Register coefficient tensors as buffers so they move with the model
        self.register_buffer('sincoeff', sin_taylor_coefficients(K, omega, D))
        self.register_buffer('coscoeff', cos_taylor_coefficients(K, omega, D))
...
class TFGNN(torch.nn.Module):
    def __init__(
        ...
        self.props = ModuleList()
        
        # First layer
        self.lins.append(Linear(in_channels, hidden_channels))
        self.props.append(Trigo_prop(K, omega, D))
    ...
    def forward(self, x, edge_index, batch, edge_weight=None):
        # Apply layers with propagation (except the last one)
        for i in range(self.num_layers - 1):
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = F.relu(self.lins[i](x))
            x = F.dropout(x, p=self.dprate, training=self.training)
            x = self.props[i](x, edge_index, edge_weight)
