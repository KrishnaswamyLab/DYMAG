...
class CayleyNet(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, r=64):
        super().__init__()
        self.convs = ModuleList()
        self.convs.append(CayleyConv(in_channels, hidden_channels, r))
        for _ in range(num_layers - 2):
            self.convs.append(CayleyConv(hidden_channels, hidden_channels, r))
...
"""
From https://github.com/anon767/CayleyNet/blob/main/CayleyNet.py
"""
import torch
...
def atan_threshold(A):
    ...
    A = torch.where(A > eps, 2.0 * torch.atan(1 / A), 2.0 * torch.ones(A.size(), device=A.device))
...
def Pmul(P, X):
    ...
    output = torch.zeros(Nout, d, Nin, device=P.device)
...
class CayleyConv(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):        
        ...
        Ut = torch.tensor(U.transpose(0, 1), device=x.device).float()
        aux = torch.mm(Ut, x)

        aux_2 = torch.tensordot(self.h, w, dims=0) 
        aux_2 = atan_threshold(aux_2)
        aux_2 = torch.tensordot(aux_2, torch.arange(1, self.r + 1, dtype=torch.float, device=x.device),
                                  dims=0) 
        ...
        output = torch.mm(torch.Tensor(U).to(x.device), out)
        output.clamp(min=0)
